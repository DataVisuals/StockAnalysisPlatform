...
import { StockData, ForecastData, NewsArticle, SearchResult, LLMPredictionData, LLMBacktestData, LLMResponse } from '../types';
...
export const searchStock = async (query: string): Promise<SearchResult> => {
  try {
    ...
    // Transform the response to match the local SearchResult type
    const apiData = response.data;
    return {
      ticker: apiData.ticker,
      company: apiData.name
    };
  } catch (error) {
